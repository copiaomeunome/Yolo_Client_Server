...
@app.route("/pegatxt", methods=["GET"])
def pegar_txt():
    return jsonify({
        "handlerId": 1,
        "videoId": 20132,
        "videoUrl": "https://example.com/video.mp4",
        "branches": ["traffic_light", "epi"],
        "cSharpCode": r"""
using System;
...
""",
        "callAIPromptAfterProcessing": False
    }), 200, {
            "Content-Type": "text/plain; charset=utf-8"
        }
    

@app.route("/onVideoProcessingFinished", methods=["POST"])
def on_video_processing_finished():
    data = request.get_json(silent=True) or {}
    video_id = data.get("videoID")
    timeline = data.get("timeline")
    prompt_reset = data.get("promptReset")

    print(f"[onVideoProcessingFinished] videoID={video_id}")
    print(f"[onVideoProcessingFinished] timeline={timeline}")
    print(f"[onVideoProcessingFinished] promptReset={prompt_reset}")
...
def run_prompt():
    ...
    print(f"[run_prompt] handlerID={handler_id}")
    print(f"[run_prompt] events={events}")

    # Dummy response; aqui você chamaria o GPT/serviço real.
    return jsonify({"response": "respostadogpt"}), 200
