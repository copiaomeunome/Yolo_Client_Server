...
def funcObservaSinalVermelho(video):
    ...
    active_ids = set()
    emitted = set()
    # usa 5% da altura (ou 10px) como deslocamento minimo para considerar subida real
    min_delta = max(int(getattr(video, "height", 0) * 0.05), 10)
    ...
    for frame in video.frames:
        ...
        for obj in frame.objects:
            if obj.nome != "red":
                continue
            frame_ids.add(obj.id)

            if obj.id not in first_y:
                first_y[obj.id] = obj.y1

            if obj.id in last_info:
                prev_y[obj.id] = last_info[obj.id][1]

            last_info[obj.id] = (t, obj.y1, obj.y2)

            # Quando o topo do objeto sai pela borda da tela
            if obj.y1 <= 0 and obj.id not in emitted:
        ...
        # ids que sumiram neste frame
        disappeared = active_ids - frame_ids
        for oid in disappeared:
            if oid in emitted:
                continue
            last_t, last_y1, last_y2 = last_info.get(oid, (None, None, None))
            if last_y1 is None:
                continue
            delta_first = first_y.get(oid, last_y1) - last_y1
            moving_up = oid in prev_y and last_y1 < prev_y[oid]
            if moving_up and delta_first >= min_delta:
                eventos.append(Event(
                    tInit=last_t,
                    tEnd=last_t,
                    name=f"Sinal vermelho saiu pelo topo (ID {oid})"
                ))
                emitted.add(oid)
